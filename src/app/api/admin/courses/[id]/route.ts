...
const updateCourseSchema = z.object({
  courseData: z.object({
    ...
    monthlyPrice: z.union([z.number(), z.string()]).transform(val => typeof val === 'string' ? parseFloat(val) || 0 : val).optional(),
    totalPrice: z.union([z.number(), z.string()]).transform(val => typeof val === 'string' ? parseFloat(val) || 0 : val).optional(),
...
export async function PUT(
  ...
  try {
    ...
    const updateData = {
      ...
      paymentType: validatedData.courseData.paymentType || existingCourse.paymentType || 'ONE_TIME',
      monthlyPrice: validatedData.courseData.monthlyPrice || existingCourse.monthlyPrice || 0,
      totalPrice: validatedData.courseData.totalPrice || existingCourse.totalPrice || 0,
