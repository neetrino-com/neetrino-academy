...
export default function SchedulePage() {
  ...
  const fetchGroupSchedule = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/groups/${selectedGroup}/schedule`)
      if (response.ok) {
        const data = await response.json()
  ...
  const generateEvents = async () => {
    ...
    setGenerating(true)
    try {
      const response = await fetch(`/api/admin/groups/${selectedGroup}/schedule/generate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex items-center justify-center">
        <div className="bg-white rounded-2xl p-8 shadow-lg">
          ...
          <p className="text-gray-600">Загрузка менеджера расписания...</p>
        </div>
      </div>
    )
  }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">Управление расписанием групп и автогенерация событий</p>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-1">{selectedGroupData?.students.length || 0}</p>
              </div>
