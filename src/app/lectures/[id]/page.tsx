          ...
          <div>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-12 mb-8 border border-gray-200">
              <div className="flex items-start justify-between mb-6">
                ...
                <Link
                  href={lecture.lessons && lecture.lessons.length > 0 
                    ? `/courses/${lecture.lessons[0].module.course.id}/lessons/${lecture.lessons[0].id}`
                    : "/courses"
                  }
                  className="flex items-center text-gray-500 hover:text-gray-700 transition-colors"
                >
                  <ArrowLeft className="w-5 h-5 mr-2" />
                  {lecture.lessons && lecture.lessons.length > 1 
                    ? `Назад к уроку (${lecture.lessons.length} уроков)`
                    : "Назад к уроку"
                  }
                </Link>
              </div>
              ...
              <div className="flex flex-wrap gap-6 text-sm text-gray-500">
                ...
                {lecture.lessons && lecture.lessons.length > 0 && (
                  <div className="flex items-center gap-2">
                    ...
                    <span>Используется в: {lecture.lessons.map(lesson => lesson.title).join(', ')}</span>
                  </div>
                )}
                <div className="flex items-center gap-2">
